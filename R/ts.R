ts<-function(records,centralValueType="median",whiskerValueType="5_95",transformationType="none") {  
  ...
  for (i in 1:length(records)) {
    ...
    return(list(dateTime[-1],dateTime[-length(records[[i]]$values)],3*mean(dateTime[-1]-dateTime[-length(records[[i]]$values)],max(c(0,3*mean(dateTime[-1]-dateTime[-length(records[[i]]$values)],trim=0.05)))))
